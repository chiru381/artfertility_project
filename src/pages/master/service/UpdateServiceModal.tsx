...
import { billOrderCompleteOptions, genderList, masterPaginationServices } from "utils/constants";
...
const UpdateServiceModal = (props: Props) => {
	...
	useEffect(() => {
		if (selectedData) {
			const findSubDepartment = departmentData?.modelItems?.find(
				(item: any) =>
					+item.id === +selectedData?.subDepartmentId
			);
			reset({
				...selectedData,
				isBillOnOrderOrComlete: selectedData?.isBillOnOrderOrComlete ? '1' : '2',
				serviceCategoryId:
					serviceCategoryOptions?.find(
						(item) => +item.value === +selectedData?.serviceCategoryId
					) ?? null,
				departmentId: departmentOptions?.find(item => +item.value === +findSubDepartment.parentDepartmentId) ?? null,
				subDepartmentId: findSubDepartment?.id ? {
					label: findSubDepartment?.name,
					value: findSubDepartment?.id,
				} : null,
				genderType:
					genderList?.find((item) => +item.value === +selectedData?.genderType) ??
					null,
				itemChargeId:
					taxOptions?.find(
						(item) => +item.value === +selectedData?.itemChargeId
					) ?? null,
			});
		}
	}, [selectedData]);
	...
	return (
		<>
			<FormModal
				...
				title={formatMessage({ id: "update-service" })}
			>
