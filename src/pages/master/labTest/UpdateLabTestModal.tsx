...
const UpdateTestModal = (props: Props) => {
    ...
    useEffect(() => {
        if (billingServiceOptions?.length && clinicOptions?.length && equipmentOptions?.length) {
            let data = {
                ...selectedData,
                billingServiceId: billingServiceOptions?.find((item: any) => item.value == selectedData?.billingServiceId) ?? [],
                testSource: testSourceOptions?.find((item: any) => item.value == selectedData?.testSource) ?? [],
                equipmentId: equipmentOptions?.find((item: any) => item.value == selectedData?.equipmentId) ?? [],
                testClinicMappingIds: selectedData?.testClinicMappings?.map((item: any) => ({ label: item.clinicName, value: item.clinicId })) ?? []
            };
            reset(data);
            setBillingServiceId(selectedData?.billingServiceId);
            setCPTCode(selectedData?.cptCode);
            setSubDepartmentName(selectedData?.subDepartmentName);
            setTestComponentMappings(testComponentMappings);

        }
    }, [reset, billingServiceOptions?.length && clinicOptions?.length && equipmentOptions?.length]);
                ...
                <div className="full-modal-container">
                    <Box className="full-modal-scroll-container">
                        <div className="full-modal-head-container">
                            <FormPrimaryHeading label={formatMessage({ id: "update-lab-test" })} />
                            <CustomFooter
