...
const UpdateTestComponentModal = (props: Props) => {
    const { closeModal, selectedData, onApiCall } = props;
    const { handleSubmit, formState: { errors }, control, watch, reset, getValues, setValue } = useForm({ mode: 'all' });
    const { formatMessage } = useIntl();
    const { toastMessage } = useToastMessage();
    const dispatch = useDispatch();
    const [loading, setLoading] = useState(false);
    const [componentResultFormulas, setComponentResultFormulas] = useState<any>([]);
    const [componentReferenceRanges, setComponentReferenceRanges] = useState<any>([]);
    const [referenceRangeCondition, setReferenceRangeCondition] = useState<any>([]);
    ...
    useEffect(() => {
        if (clinicOptions?.length && sampleTypeOptions?.length && unitOfMeasureOptions?.length
            && sampleContainerOptions?.length && resultValueOptions?.length && subDepartmentOptions?.length) {
            let clinicIds = selectedData?.componentClinicMappings?.map((item: any) => { return item.clinicId });
            let resultValueRecordIds = selectedData?.componentResultValues?.map((item: any) => { return item.resultValueId });

            let data = {
                ...selectedData,
                componentClinicMappingIds: clinicOptions?.filter((item: any) => clinicIds?.includes(parseInt(item.value))) ?? null,
                sampleId: sampleTypeOptions?.find((item: any) => item.value == selectedData?.sampleId) ?? null,
                unitOfMeasureId: unitOfMeasureOptions?.find((item: any) => item.value == selectedData?.unitOfMeasureId) ?? null,
                sampleContainerId: sampleContainerOptions?.find((item: any) => item.value == selectedData?.sampleContainerId) ?? null,

                componentResultValueIds: resultValueOptions?.filter((item: any) => resultValueRecordIds?.includes(parseInt(item.value))) ?? null,

                subDepartmentId: subDepartmentOptions?.find((item: any) => item.value == selectedData?.subDepartmentId) ?? null,
                resultType: resultTypeOptions?.find((item: any) => item.value == selectedData?.resultType) ?? null,
                formulaName: selectedData?.componentResultFormulas[0]?.formulaName,
                formulaDecimalUpto: selectedData?.componentResultFormulas[0]?.decimalUpto,
                formulaIsRoudingOff: selectedData?.componentResultFormulas[0]?.isRoudingOff,
            };

            setReferenceRangeCondition(referenceRangeConditionOptions?.find((item: any) => item.value == selectedData?.componentReferenceRanges[0]?.conditionAgeGender ?? 0) ?? [])
            setComponentResultFormulas(selectedData?.componentResultFormulas);

            const rangeData = selectedData?.componentReferenceRanges?.map((item: any) => ({
                ...item,
                genderId: genderTypeOptions.find((option: any) => option.value == item.genderId)
            }));

            setComponentReferenceRanges(rangeData);

            reset({
                ...data,
                'componentReferenceRanges': rangeData
            });
        }
    }, [reset, clinicOptions?.length && sampleTypeOptions?.length && unitOfMeasureOptions?.length
        && sampleContainerOptions?.length && resultValueOptions?.length && subDepartmentOptions?.length]);
    ...
    function onSubmit(data: any) {
        ...
        setLoading(true);
        services.updateComponent(bodyData)
            .then((res) => {
                setLoading(false);
...
export default UpdateTestComponentModal;
