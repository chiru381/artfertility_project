...
import { CustomDatePicker, CustomSelect, CustomTextBox, CustomTimePicker } from 'components/forms';
...
interface Props {
    ...
    onCurrentDateChange: (date: any) => void;
}

const RescheduleAppointmentModal = (props: Props) => {
    const { closeModal, onApiCall, selectedAppointment, onCurrentDateChange } = props;
    ...
    const { appointmentLookupData, resourceAppointmentSlotData, appointmentByDateData, doctorSlotBlockData } = useSelector(
    ...
    useEffect(() => {
        resourceAppointmentSlotData.modelItems.map((slot: any) => {
            ...
            let days = slot.resourceSlotConfigDetails.filter((slotConfig: any) => slotConfig.isExcluded).map((slotConfig: any) => slotConfig.dayOfWeekNumber);
    ...
    function onSubmit({ fromTime, toTime, rescheduleRemark, rescheduleReasonId }: any) {
        let resourceId = selectedAppointment?.resourceId;
        ...
        if ((isSlotBlocked || checkOverlapped)) {
            ...
            let bodyData = getFormBody({ rescheduleRemark, rescheduleReasonId });
            let formData = {
                ...bodyData,
                appointmentId: selectedAppointment.id,
                isRescheduled: true,
                resourceId: selectedAppointment.resourceId,
                appointmentCallTypeId: selectedAppointment.appointmentCallTypeId,
                clinicId: selectedAppointment.clinicId,
                medicalStaffId: selectedAppointment.medicalStaffId,
            ...
            setLoading(true);
            services.rescheduleAppointment(formData)
                .then((res) => {
                    setLoading(false);
                ...
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12}>
                        <CustomDatePicker
                            ...
                            rules={{ required: true }}
                            onChangeValue={date => onCurrentDateChange(date)}
                        />
                    </Grid>
