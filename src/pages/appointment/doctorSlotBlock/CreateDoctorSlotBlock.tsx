...
const CreateDoctorSlotBlock = ({ closeModal, selectedData, onApiCall }: Props) => {
    ...
    function updateFields(data: any) {
        const { blockTypeId, blockReasonId, medicalStaffId, fromDate, toDate, fromTime, toTime } = data;
        ...
        setValue('medicalStaffId', selectOptions?.medicalStaffs?.find((list: any) => +list.value === +medicalStaffId) ?? null);
    ...
    function onSubmit(data: any) {
        ...
        setLoading(true);
        let doctorSlotBlockService = services[(selectedData ? 'updateDoctorSlotBlock' : 'createDoctorSlotBlock') as keyof typeof services];
        doctorSlotBlockService(selectedData ? { ...formData, id: selectedData.id } : formData)
            .then((res) => {
                setLoading(false);
                if (res.data?.succeeded) {
                    toastMessage(formatMessage({ id: selectedData ? "update-doctor-slot-block-message" : "create-doctor-slot-block-message" }));
                    closeModal();
                    ...
                    <Box p={2} style={{ borderBottom: "1px solid rgba(0,0,0,0.1)" }}>
                        <Grid spacing={2} container >
                            <Grid item xs={12} lg={3} md={4} sm={6}>
                                <CustomSelect
                                    label={formatMessage({ id: "select-doctor" })}
...
export default CreateDoctorSlotBlock;