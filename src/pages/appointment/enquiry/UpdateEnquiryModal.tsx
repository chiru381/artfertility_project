...
const UpdateEnquiryModal = React.memo(({ closeModal, onApiCall, selectedData }: Props) => {
    const { handleSubmit, formState: { errors }, control, setValue, getValues, watch } = useForm({ mode: 'all' });
    ...
    function updateFields(data: any) {
        const { patientAgeInYear, resourceId, appointmentNumber } = data;
        let formData = getValues();
        let formatedFormData: any = {};

        Object.keys(formData).map(key => {
            if (data[key] && key.includes("Id")) {
        ...
        setAge(patientAgeInYear);
        setEnquiryNum(appointmentNumber);
        setValue('resourceId', selectOptions?.resources?.find((resource: SelectOptionsState) => +resource.value === resourceId) ?? null);
    }
    ...
    function onSubmit({ fromTime, toTime, chnid, ...rest }: any) {
        ...
        services.updateAppointment(bodyData)
            .then((res) => {
                setCreateBookingLoading(false);
                if (res.data?.succeeded) {
                    closeModal();
                    toastMessage(formatMessage({ id: "update-enquiry-message" }));
                    onApiCall();
                } else {
    ...
    return (
        <>
            <FormModal
                ...
                confirmLabel={formatMessage({ id: "update" })}
                modalSize="medium"
            >
